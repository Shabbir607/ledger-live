...
const Send = () => {
    ...
    const fetchWallets = async () => {
      try {
        ...
        if (data.success && data.wallets) {
          ...
          const firstWithBalance = data.wallets.find(
            (w) => parseBalance(w.balance) > 0
          );
  ...
  const handleSend = async () => {
    ...
    try {
      const payload = {
        ...
        type: selectedWallet?.wallet_type || "ETH",
        ...(memo ? { memo } : {}),
      };
      ...
      if (result.success) {
        setSuccess(
          `Successfully sent ${amount} ${selectedWallet.wallet_type}!`
        );
        setAmount("");
        setRecipientAddress("");
        setMemo("");
        ...
        if (refreshData.success && refreshData.wallets) {
          ...
          const updatedWallet = refreshData.wallets.find(
            (w) => w.wallet_type === selectedWallet.wallet_type
          );
          setSelectedWallet(updatedWallet);
        }
      } else {
        const addressErrors = result?.errors?.to_wallet_address;
  ...
  // Open confirmation dialog with cooldown to prevent accidental sends
  const handleReview = () => {
  ...
  // Countdown timer for the confirm dialog
  useEffect(() => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          ...
          >
            <div className="space-y-1">
              ...
              >
                Please confirm the destination address before sending.
              </p>
            </div>
            ...
            >
              <div>
                ...
                >
                  {amount || "0"} {selectedWallet?.wallet_type}
                </p>
              </div>
              <div
                className={cn(
              ...
              >
                From {selectedWallet?.wallet_type}
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-2">
              ...
              <Button
                ...
                disabled={confirmCountdown > 0 || sending}
          ...
          <div className="relative">
            ...
            >
              ...
              {wallets.length > 1 && (
                <ChevronDown
                  className={
                    darkMode ? "w-5 h-5 text-gray-400" : "w-5 h-5 text-gray-600"
                  }
                />
              )}
            </button>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-left flex-1 min-w-0">
                        ...
                        >
                          {formatBalance(wallet.balance, 2)}{" "}
                          {wallet.wallet_type}
                        </p>
                      </div>
                      {isDisabled && (
